...
def test_ridge_likelihood():
    ...
    for noise_variance in [0.35, 1.1, 3.0]:
      ...
      assert jnp.isclose(est_eigvals[0], lambda_1, rtol=1e-2)
      assert jnp.isclose(est_eigvals[1], lambda_2, rtol=1e-2)

      # Check that recovered eigenvectors are aligned with a1 and a2 (up to sign)
      dot1 = jnp.abs(jnp.dot(est_eigvecs[:, 0], a1))
      dot2 = jnp.abs(jnp.dot(est_eigvecs[:, 1], a2))
      assert jnp.isclose(dot1, 1.0, atol=1e-2) or jnp.isclose(dot2, 1.0, atol=1e-2)
      assert jnp.isclose(dot1, 1.0, atol=1e-2) or jnp.isclose(dot2, 1.0, atol=1e-2)

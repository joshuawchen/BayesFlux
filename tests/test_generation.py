...
class ExampleSampler(GaussianInputOuputAndDerivativesSampler):
        def __init__(self):
            ...
            self._precision, self._L2_inner_product_matrix = np.identity(self._input_dimension), np.identity(self._input_dimension)
        ...
        def _value(self, input_sample: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
            return input_sample[0:-1]**3. #f(x) = x*3, pointwise cube, except for the last dimension of x
        ...
        def value_and_matrix_jacobian_prod(self, input_sample: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
            ...
            jacobian_sample[0:-1] = (3*input_sample[0:-1]**2.)[:,None]
...
def test_generate_reduced_training_data():
    sampler = ExampleSampler()
    results = generate_reduced_training_data(sampler_wrapper=sampler, 
                                             N_samples=12,
